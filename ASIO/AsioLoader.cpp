...
bool AsioLoader::LoadDriver(char *name)
{
    if (m_pAsioDriver)
        return m_pAsioDriver->loadDriver(name);
    return false;
}

ASIOTime* AsioLoader::bufferSwitchTimeInfo(ASIOTime *timeInfo, long index, ASIOBool processNow)
{
    ...
    {
        // skip inputs
        if (asioDriverInfo.bufferInfos[bufferIndex].isInput)
            continue;
...
//----------------------------------------------------------------------------------
void AsioLoader::bufferSwitch(long index, ASIOBool processNow)
{
    // the actual processing callback.
    // Beware that this is normally in a seperate thread, hence be sure that you take care
    // about thread synchronization. This is omitted here for simplicity.
...
void AsioLoader::sampleRateChanged(ASIOSampleRate sRate)
{
...
long AsioLoader::asioMessages(long selector, long value, void* message, double* opt)
{
...
long AsioLoader::Init(DriverInfo *asioDriverInfo)
{
...
void AsioLoader::SetupCallbacks()
{
...
ASIOError AsioLoader::CreateBuffers(DriverInfo *asioDriverInfo)
{
